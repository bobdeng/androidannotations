...
public class BeanHandler extends BaseAnnotationHandler<EComponentHolder> {
	...
	public void process(Element element, EComponentHolder holder) throws Exception {
		...
		JInvocation getInstance = injectedClass.staticInvoke(EBeanHolder.GET_INSTANCE_METHOD_NAME).arg(holder.getContextRef()).arg(_this());
        Bean beanAnnotation = element.getAnnotation(Bean.class);
		block.assign(beanField, getInstance);
        TypeMirror typeMirrorInvocationHandler = annotationHelper.extractAnnotationParameter(element, "invocationHandler");
        if(!typeMirrorInvocationHandler.toString().equals(Void.class.getName())){
            block.assign(beanField,cast(refClass(element.asType().toString()),refClass(typeMirrorInvocationHandler.toString()).staticInvoke("newInstance").arg(beanField)));
        }
	}
}
